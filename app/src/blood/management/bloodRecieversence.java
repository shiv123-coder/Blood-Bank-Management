...
public class bloodRecieversence implements Initializable {
    ...
    private void addReciever(ActionEvent event) {
        ...
        try {
            ...
            String query = "insert into RECIEVERTable (date,Name,BloodGroup,Gender,Amount,Qty,DoctorName,Eid,Contact) values ('"
                    + date + "','" + name + "','" + bGroup + "','" + gender + "','" + amount + "','" + qty + "','"
                    + doctor + "','" + eid + "','" + contact + "');";
    ...
    private void mDeleteRecord(ActionEvent event) {
        ...
        try {
            c = DBConnection.connect();
            String query = "Update InventoryTable set Qty=Qty+" + Integer.parseInt(row.get(8)) + " where bloodgroup='"
                    + row.get(3) + "';";
            c.createStatement().execute(query);
            c.close();
