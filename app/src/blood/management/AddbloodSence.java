...
public class AddbloodSence implements Initializable {
    ...
    private void addDonor(ActionEvent event) {
        ...
        try {
            if (!update) {
                ...
                query = "insert into BloodDonorTable (Name,Gender,Address,date,BloodGroup,EID,QTY,Amount) values ('"
                        + name + "','" + gender + "','" + address + "','" + aDate + "','" + bGroup + "','" + eid + "','"
                        + qty + "','" + amount + "');";
                ...
                query = "Update BloodDonorTable set Name='" + name + "',Gender='" + gender + "',Address='" + address
                        + "',"
                        + "date='" + aDate + "',BloodGroup='" + bGroup + "',EID='" + eid + "',QTY='" + qty
            ...
            if (update) {
                ...
                ResultSet rs = QueryDatabase
                        .query("Select bloodGroup from inventorytable where bloodgroup='" + bGroup + "';");
                if (rs == null) {
    ...
    private void mDeleteDonor(ActionEvent event) {
        ...
        try {
            c = DBConnection.connect();
            String query = "Update InventoryTable set Qty=Qty-" + Integer.parseInt(row.get(7)) + " where bloodgroup='"
                    + row.get(5) + "';";
            c.createStatement().execute(query);
            c.close();
